...
function vehicleEntrySortValue(entry) {
  return new Date(entry.updatedAt || entry.createdAt || entry.fuelDate || 0).getTime();
}
...
async function saveEntry(syncAfter) {
  ...
  toast((existing >= 0 ? 'Fuel entry updated locally.' : 'Fuel entry saved locally.') + ' Sync will run automatically.', 'success');
  await syncEntryToSheet(state.entries.find((item) => item.id === id), { silent: true });
  resetEntryForm();
...
function renderAlerts(data) {
  ...
  $('alertList').innerHTML = [
    ...
    ['Google Sheets status', sheetUrl() ? 'Connected' : 'Not configured', sheetUrl() ? 'success' : 'warning']
...
function mergeCentralWithPending(centralEntries, pendingEntries) {
  const map = new Map();
  centralEntries.forEach((entry) => map.set(entry.id || uid(), { ...entry, syncStatus: entry.syncStatus === 'Pending Sync' ? 'Pending Sync' : 'Synced', syncError: entry.syncError || '' }));
  pendingEntries.forEach((entry) => {
    const key = entry.id || uid();
    if (!map.has(key) || entry.syncStatus === 'Pending Sync') map.set(key, entry);
  });
...
async function syncEntryToSheet(entry, options = {}) {
  ...
  if (!url) {
    markEntryPending(entry, 'Google Sheet Not Connected');
    if (!options.silent) toast('Google Sheet Not Connected. Entry remains Pending Sync.', 'warning');
    return false;
  }
  try {
...
async function syncPendingEntries(showToast = false) {
  ...
  if (!url) {
    if (showToast) toast('Google Sheet Not Connected. Pending data remains local.', 'warning');
    return;
  }
  let synced = 0;
  for (const entry of pending) {
    if (await syncEntryToSheet(entry, { silent: true, refresh: false })) synced++;
  }
  if (synced) audit('Synced pending entries', synced + ' rows');
  if (synced) await loadCentralData({ showToast: false, reason: 'pending-sync' });
  updateSheetStatus();
  if (state.page === 'fuel-entries') renderEntries();
  if (state.page === 'settings') renderSheetConnectionStatus();
  if (showToast) toast(`${synced} of ${pending.length} pending entries synced.`, synced === pending.length ? 'success' : 'warning');
}
...
async function testSheetConnection() {
  ...
  if (!url) return toast('Enter Google Apps Script Web App URL.', 'danger');
  try {
    await postToSheet(url, { action: 'testConnection', sheet: APP_CONFIG.googleSheetName });
...
function pendingSyncEntries() {
  return state.entries.filter((entry) => entry.syncStatus === 'Pending Sync');
}
...
function entriesForUser() {
  ...
  if (perms.seeAllDistricts || !state.user?.district) return state.entries;
  return state.entries.filter((e) => e.district === state.user.district);
}
...
function updateSheetStatus() {
  const connected = !!sheetUrl();
  const pending = pendingSyncEntries().length;
  const last = state.settings.lastSyncTime ? dateTime(state.settings.lastSyncTime) : '-';
  $('sheetStatus').textContent = `${connected ? 'Google Sheet Connected' : 'Google Sheet Not Connected'} | Pending Sync: ${pending} | Last Sync: ${last}`;
...
function isoFromSheet(value) {
  ...
  return Number.isNaN(parsed.getTime()) ? String(value).slice(0, 10) : isoDate(parsed);
}
