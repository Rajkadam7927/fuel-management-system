...
function loadState() {
  ...
  if (cachedConnection?.googleScriptUrl && !state.settings.globalGoogleScriptUrl) state.settings.globalGoogleScriptUrl = cachedConnection.googleScriptUrl;
...
async function syncPendingEntries(showToast = false) {
  ...
  for (const entry of pending) {
    if (await syncEntryToSheet(entry, { silent: true })) synced++;
  }
...
function sheetUrl() {
  return state.settings.globalGoogleScriptUrl || state.settings.googleScriptUrl || APP_CONFIG.googleScriptUrl || '';
}
